  ...
  function setSliderControlsHeight() {
    ...
    $('#shopify-section-1550222669998 .slider--next, #shopify-section-1550222669998 .slider--prev').css({
  ...
  function resizeCustomContentHeight() {
    var customContentHeight = $('.custom-content .grid-container > div:nth-child(2) img').height();
    $('.custom-content .grid-container > div:nth-child(4) > div, .custom-content .grid-container > div:nth-child(1) > div').css('height', customContentHeight);
  }
  ...
  $('.products-grid .product-elemet').each(function() {
      ...
      $(this).find('.product-item').append('<a class="product-btn" href="'+cartLink+'">Add to Cart</a>');
  });
  ...
  function fuckSwiper() {
    $('.template-product .product-slider .swiper-wrapper').attr('style', 'transform:translate3d(0px, 0px, 0px)');
  }
  fuckSwiper();
    
  // Clicks Swiper Slider Controls
  $('.product-thumbnail-nav-next, .product-gallery.carousel .carousel-nav-next, .product-thumbnail-nav-prev, .product-gallery.carousel .carousel-nav-prev').on('click', function() {
  var loopOneActive = $('.product-slider .swiper-wrapper > div').eq(totalSlides).hasClass('swiper-slide-active');
  var loopTwoActive = $('.product-slider .swiper-wrapper > div').eq(totalSlides*2).hasClass('swiper-slide-active');
  ...
  setTimeout(function() {
    ...
    var swiper = new Swiper('.product-slider .swiper-container', {
      ...
      // Navigation arrows
      navigation: {
        nextEl: '.product-thumbnail-nav-next, .product-gallery.carousel .carousel-nav-next',
        prevEl: '.product-thumbnail-nav-prev, .product-gallery.carousel .carousel-nav-prev',
      },
      scrollbar: {
  ...
  // On Next Click
  $('.product-thumbnail-nav-next, .product-gallery.carousel .carousel-nav-next').on('click', function() {
      ...
      var currentSlideIndex = $('.product-slider .swiper-slide-active').attr('data-swiper-slide-index');
      
      console.log('next', transX);
  
      if(currentSlideIndex == totalSlides) {
          setTimeout(function() {
              $('.product-slider .swiper-wrapper').attr('style', 'transform: translate3d('+transX - 208+'px, 0px, 0px)');
          }, 1200);
          console.log('activated');
      }
...
$('body.template-index .product-item').each(function() {
    ...
    $(this).find('.product-item-price').after('<div class="product-item-button"><a href="'+productLink+'">Detail</a></div>');
